...
hidden_size = 50
num_layers = 1
num_epochs = 20  # Number of epochs for each federated round
...
for round in range(num_rounds):
    ...
    for path in client_data_paths:
        ...
        # Split data into 5 parts and select the part corresponding to the current round
        total_parts = 5
        part_size = len(df) // total_parts
        start_idx = (round % total_parts) * part_size
        end_idx = start_idx + part_size if (start_idx + part_size) < len(df) else len(df)
        df_part = df.iloc[start_idx:end_idx]

        X, y = create_sequences(df_part['speed'].values, sequence_length)
